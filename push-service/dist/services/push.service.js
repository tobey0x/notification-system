...
class PushService {
    constructor() {
        ...
        this.CIRCUIT_RESET_TIMEOUT = 60000; // 1 minute
     ...
     */
    async sendToUser(userId, notification, notificationId = (0, uuid_1.v4)(), retryCount = 0) {
        // Check circuit breaker
        if (this.isCircuitOpen) {
            logger_1.logger.warn('Circuit breaker is open, notification queued for retry later');
        ...
        try {
            ...
            if (!user?.preferences?.push || !user.push_token) {
                logger_1.logger.warn(`User ${userId} has push notifications disabled or no token`);
     ...
     */
    async sendToDevice(token, notification, notificationId = (0, uuid_1.v4)()) {
        try {
            ...
            logger_1.logger.info(`Push notification sent successfully`, { notificationId, token: this.maskToken(token) });
            return true;
        }
        catch (error) {
            logger_1.logger.error('Error sending push notification', {
                error: error.message,
                notificationId,
    ...
    async getUserData(userId) {
        try {
            const response = await axios_1.default.get(`${USER_SERVICE_URL}/api/v1/users/${userId}`, {
    ...
    async getTemplate(templateCode) {
        try {
            const response = await axios_1.default.get(`${TEMPLATE_SERVICE_URL}/api/v1/templates/${templateCode}`, {
                headers: { 'Content-Type': 'application/json' },
                timeout: 5000, // 5 seconds timeout
            });
            return response.data;
            ...
            logger_1.logger.error('Error fetching template', {
                templateCode,
                error: error.message,
                status: error.response?.status,
            });
     ...
     */
    async processNotificationFromQueue(message) {
        ...
        try {
            ...
            const notificationPriority = priority && priority > 5 ? 'high' : 'normal';
            ...
            logger_1.logger.error('Error processing notification from queue', {
                user_id,
                template_code,
                request_id,
                error: error.message,
    ...
    async updateNotificationStatus(notificationId, status, error) {
        try {
            ...
            await axios_1.default.post(`${API_GATEWAY_URL}/api/v1/notification/status`, update, { timeout: 3000 });
        }
        catch (error) {
            logger_1.logger.error('Error updating notification status', {
                notificationId,
                status,
                error: error.message,
            });
     ...
     */
    async handleError(error, notificationId, userId, notification, retryCount) {
        const errorMessage = error.message || 'Unknown error';
        // Log the error
        logger_1.logger.error('Push notification error', {
            notificationId,
            userId,
            error: errorMessage,
            retryCount,
        ...
        if (retryCount >= MAX_RETRIES) {
            await this.updateNotificationStatus(notificationId, 'failed', `Max retries reached: ${errorMessage}`);
            return;
        }
        ...
        logger_1.logger.info(`Scheduling retry ${retryCount + 1} in ${delay}ms`, { notificationId });
        setTimeout(async () => {
    ...
    tripCircuitBreaker() {
        ...
        logger_1.logger.error('Circuit breaker tripped - push notifications paused');
    ...
    resetCircuitBreaker() {
        this.isCircuitOpen = false;
        this.failureCount = 0;
        logger_1.logger.info('Circuit breaker reset - push notifications resumed');
    }
    /**
    ...
    async sendToTopic(topic, notification) {
        try {
            ...
            logger_1.logger.info(`Successfully sent message to topic ${topic}`);
            return true;
        }
        catch (error) {
            logger_1.logger.error(`Error sending push notification to topic ${topic}:`, {
                error: error.message,
                stack: error.stack,
            });
            throw error;
    ...
    async subscribeToTopic(tokens, topic) {
        try {
            ...
            logger_1.logger.info(`Successfully subscribed to topic: ${topic}`);
        }
        catch (error) {
            logger_1.logger.error(`Error subscribing to topic ${topic}:`, error);
            throw error;
        }
    }
    async unsubscribeFromTopic(tokens, topic) {
        try {
            ...
            logger_1.logger.info(`Successfully unsubscribed from topic: ${topic}`);
        }
        catch (error) {
            logger_1.logger.error(`Error unsubscribing from topic ${topic}:`, error);
            throw error;
        }
    }
}
